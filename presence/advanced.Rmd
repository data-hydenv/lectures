...
select 
  all_idx.meta_id, all_idx.date, all_idx.mean as "mean.all", day_idx.mean as "mean.day" 
FROM temperature_indices as all_idx
JOIN day_temperature_indices as day_idx on day_idx.meta_id=all_idx.meta_id and day_idx.date=all_idx.date
WHERE all_idx.date > '2020-11-01' and all_idx.date < '2021-02-1'
...
```
```{sql connection=con}
SELECT str_value as value, count(*) as "n_references" from nm_metadata_details nm
JOIN details d ON d.id=nm.detail_id
WHERE key='exposition'
GROUP BY value
```
...
```{sql connection=con}
select m.*, exp.exposition, inf.influence
FROM metadata m
JOIN (
  select meta_id, str_value as exposition FROM nm_metadata_details nm
  JOIN details d on nm.detail_id=d.id
  where d.key='exposition'
) exp on exp.meta_id=m.id
JOIN (
  select meta_id, str_value as influence FROM nm_metadata_details nm
  JOIN details d on nm.detail_id=d.id
  where d.key='influence'
) inf on inf.meta_id=m.id
```
...
```{sql connection=con}
select exposition, influence, avg(value) as mean_t from raw_data
JOIN (
  select m.*, exp.exposition, inf.influence
  FROM metadata m
  JOIN (
...
) as m on m.id=raw_data.meta_id
where variable_id = 1
Group by exposition, influence
order by exposition, influence
```
...
```{sql connection=con}
select meta.camels_id, federal_state, area, data.count from camels_de_metadata meta
LEFT JOIN (
  select camels_id, count(*) from camels_de_data
  group by camels_id
) as data on data.camels_id=meta.camels_id
```
...
```{sql connection=con}
select small_meta.camels_id, federal_state, area, data.count from small_meta
LEFT OUTER JOIN (
  select camels_id, count(*) from camels_de_data
  group by camels_id
) as data on data.camels_id=small_meta.camels_id
```
