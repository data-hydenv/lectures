...
```{sql connection=con}
SELECT id, hobo_id, year FROM hobo WHERE year=2018
```
...
```{sql connection=con}
select year, count(year) from hobo group by year
```
...
select * from raw_data
  join hobo on raw_data.hobo_id=hobo.hobo_id
...
```{sql connection=con}
select h.hobo_id, radiation_influence, count(tstamp) as amount, avg(temperature) as mean_t, avg(light) as mean_light 
from raw_data r
join hobo h on h.hobo_id=r.hobo_id
group by h.hobo_id, radiation_influence
```
