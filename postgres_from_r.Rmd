...
You can find the 'hobo' table with all the HOBO sensor usages from the last years and a 'raw_data' table containing the measurements.
Load the hobo table to a R data.frame:
```{r}
hobo <- dbReadTable(con, 'hobo')
hobo
```
...
```{r}
import <- data.frame(id=9999, hobo_id=999999, radiation_influence="high",geom="SRID=4326;POINT(7.8 48.0)", description="deleteme", year=9999)
names(import) <- names(hobo)
dbWriteTable(con, 'hobo', import, append=TRUE, row.names=FALSE)
```
...
```{r}
sql <- 'Select * from hobo where id=9999'
dbGetQuery(con, sql)
```
...
RStudio Markdown files are also a great way of working with databases. Beside a code chunk in R, it can also handle SQL chunks. All we need is the dbConnection object referenced in the 'con' variable. All we need to do, is placing the SQL directly into the chunk and RStudio will do the rest for us. <br>
Although this makes coding on databases easy and improves the readability of the file, be aware of the actual commands executed here in case you ever have to build a script without RStudio or debug one. Do not get too dependend on a specific IDE, even if it's a great IDE. In real world applications, R is usually not the language to be used to database management. It's either a specific IDE, C++ or Python.<br>
However, the code from the last chunk could also be run like:

```{sql connection=con}
select * from hobo where id=9999
```
...
```{sql connection=con}
delete from hobo where id=9999
```
